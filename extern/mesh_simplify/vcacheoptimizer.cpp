...
void meshopt_optimizeVertexCacheTable(unsigned int* destination, const unsigned int* indices,
									  unsigned int* destination_parents, unsigned int* indices_parents, size_t index_count, 
	...
	// vertices parents
	if (destination_parents == indices_parents)
	{
		unsigned int* indices_parents_copy = allocator.allocate<unsigned int>(index_count);
		memcpy(indices_parents_copy, indices_parents, index_count * sizeof(unsigned int));
		indices_parents = indices_parents_copy;	
	}
