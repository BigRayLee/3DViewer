...
/* Remap vertex and uv coordinates */
void meshopt_remapVertexBufferwithUV(void* destination_verts, const void* vertices,
									 void* destination_uvs, const void* uvs,  
	...
	// support in-place remap
	if (destination_verts == vertices)
	{
		unsigned char* vertices_copy = allocator.allocate<unsigned char>(vertex_count * vertex_size);
		memcpy(vertices_copy, vertices, vertex_count * vertex_size);
		vertices = vertices_copy;
	}
	if (destination_uvs == uvs)
	{
		unsigned char* uvs_copy = allocator.allocate<unsigned char>(vertex_count * 2 * sizeof(float));
		memcpy(uvs_copy, uvs, vertex_count * 2 * sizeof(float));
		uvs = uvs_copy;
	}
	...
	for (size_t i = 0; i < vertex_count; ++i)
	{
		if (remap[i] != ~0u)
		{
			...
			memcpy(static_cast<unsigned char*>(destination_verts) + remap[i] * vertex_size, static_cast<const unsigned char*>(vertices) + i * vertex_size, vertex_size);
			memcpy(static_cast<unsigned char*>(destination_uvs) + remap[i] * 2 * sizeof(float), static_cast<const unsigned char*>(uvs) + i * 2 * sizeof(float), 2 * sizeof(float));
		}
	}

}

/* Gnerate vertex and uv remap */
size_t meshopt_generateVertexRemapwithUV(unsigned int* destination, const unsigned int* indices, 
				size_t index_count, const void* vertices, size_t vertex_count, const void* uvs, 
				size_t vertex_size){
	using namespace meshopt;
	...
	void* vert_uv = malloc(vertex_count * 5 * sizeof(float));

	for(int i = 0; i < vertex_count; ++i){
		memcpy(static_cast<unsigned char*>(vert_uv) + 5 * i * sizeof(float), (unsigned char*)(vertices) + 3 * i * sizeof(float), sizeof(float) * 3);
		memcpy(static_cast<unsigned char*>(vert_uv) + 5 * i * sizeof(float) + 3 * sizeof(float), (unsigned char*)(uvs) + 2 * i * sizeof(float), sizeof(float) * 2);
	}

	VertexHasher hasher = {static_cast<const unsigned char*>(vert_uv), vertex_size, vertex_size};
