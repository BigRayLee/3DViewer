...
namespace ImGui
{
    ...
    IMGUI_API void          MemoryFree(void* ptr);
...
struct ImVector
{
    ...
    inline ~ImVector()                                      { if (Data) ImGui::MemoryFree(Data); }
    ...
    inline void         clear()                             { if (Data) { Size = Capacity = 0; ImGui::MemoryFree(Data); Data = NULL; } }
    ...
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = (T*)ImGui::MemAlloc((size_t)new_capacity * sizeof(T)); if (Data) { memcpy(new_data, Data, (size_t)Size * sizeof(T)); ImGui::MemoryFree(Data); } Data = new_data; Capacity = new_capacity; }
...
struct ImGuiIO
{
    ...
    int         MetricsActiveAllocations;       // Number of active allocations, updated by MemAlloc/MemoryFree based on current context. May be off if you have multiple imgui contexts.
...
// Helper: IM_NEW(), IM_PLACEMENT_NEW(), IM_DELETE() macros to call MemAlloc + Placement New, Placement Delete + MemoryFree
...
template<typename T> void IM_DELETE(T* p)   { if (p) { p->~T(); ImGui::MemoryFree(p); } }
