...
void main(){
    ...
    if(params.isAdaptive)
        //gl = vec4(aPos, 1.0);
        gl = (blend) * vec4(aPos, 1.0) + (1 - blend) * vec4(aPos_ssbo[p], aPos_ssbo[p + 1], aPos_ssbo[p + 2], 1.0);
    else
        gl = vec4(aPos, 1.0);
    ...
    /*out variables*/
    Normal = blend * aNorm_in + (1 - blend) * vec3(aNorm_in_p[p], aNorm_in_p[p + 1], aNorm_in_p[p + 2]);
    ViewDir = params.vp - (matrices.model * gl).xyz;
    LightPos = ViewDir;
    
    /*color information*/
    //Color = blend * vec3(color_in.x/255.0, color_in.y/255.0, color_in.z/255.0) + 
    //        (1 - blend) * vec3(color_in_p[color_idx_p].x/255.0, color_in_p[color_idx_p].y/255.0, color_in_p[color_idx_p].z/255.0);
